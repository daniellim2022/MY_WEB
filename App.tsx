...
const App: React.FC = () => {
  ...
  const handleDeleteLink = (id: string) => {
    if (window.confirm('정말로 이 사이트를 삭제하시겠습니까?')) {
      ...
      // If deleted link was selected, clear selection
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <Navbar />
      <main className="flex-grow flex flex-col px-4 sm:px-6 lg:px-8 py-8">
        ...
        {/* Main Content Area: Cards + iframe */}
        <div className="flex-grow flex gap-6 overflow-hidden min-h-0">
          {/* Left: Website Cards */}
          <div className={`transition-all duration-300 flex flex-col ${selectedUrl ? 'w-1/3 min-w-[320px] max-w-[400px]' : 'w-full'}`}>
          ...
          {/* Right: iframe */}
          {selectedUrl && (
            <div className="flex-1 min-w-0 bg-white rounded-lg shadow-lg overflow-hidden flex flex-col">
              <WebViewer url={selectedUrl} />
            </div>
          )}
        </div>
      </main>
